...
Event (immutable, append-only)
  ...
  ├─ event_kind       proposed | accepted | dismissed | applied-change
...
```
proposed ─┬─▶ accepted ──▶ (applied-change event carries before/after)
          └─▶ dismissed (carries dismissal reason; never produces a change)
```
...
| Attribution integrity | `actor_kind`/`source` set by handlers; applied-suggestion attribution comes from client `ai_applied` flag | **Forgeable** (§6) |
...
- Threat is low: authenticated users editing their own maps; a forged flag mislabels provenance on the user's *own* change log — no privilege or trust boundary is crossed.
