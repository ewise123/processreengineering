...
- **#6 manual lane-move 422** — INVESTIGATED, no code bug: the reason IS threaded (panel → `updateNodeImpl` → reason dialog → PATCH with reason; live PATCH confirmed with-reason→200). Likely a stale bundle during the test. Re-test on a fresh session.
