...
IMPL_PLAN_TAIL_PROMPT_BASE = """You are a business transformation consultant. Generate the following sections of an implementation plan in markdown format.
...
def _generate_implementation_plan(api_key: str, document_text: str,
    ...
    # ── Call 2: value_add + sources in their own isolated call ───────────────
    if want_value_add or want_sources:
        tail_sections_list = (
            (['value_add'] if want_value_add else []) +
            (['sources']   if want_sources   else [])
        )
        # Build section descriptions using the existing IMPL_PLAN_SECTIONS data
        tail_blocks = []
        for sid, title, desc in IMPL_PLAN_SECTIONS:
            if sid in tail_sections_list:
                tail_blocks.append(f"## {title}\n{desc}")
        sections_block = "\n\n".join(tail_blocks)
        tail_sys = IMPL_PLAN_TAIL_PROMPT_BASE.format(sections_block=sections_block)

        tail_user = (
            f"Generate the sections listed in your instructions for this process:\n\n"
            f"- Current State Process: {current_process}\n"
            f"- Future State Process: {future_process}\n\n"
            f"Source documents:\n\n{document_text[:8000]}\n\n"
            f"Implementation plan already written (use for assumption cross-referencing only):\n\n"
            f"{parts[0][:3000] if parts else '(none)'}"
        )
